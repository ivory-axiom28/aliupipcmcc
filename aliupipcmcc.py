...
def fetch_telecom_ips():
    print(f"{get_beijing_time()} - 开始提取移动IP...")
    ...
    for row in soup.find_all("tr"):
        ...
        if len(cols) >= 2 and cols[0].get_text(strip=True) == "移动":
    ...
    print(f"{get_beijing_time()} - 提取完成，共 {len(ips)} 个移动 IP。已保存到 {OUTPUT_FILE}")
